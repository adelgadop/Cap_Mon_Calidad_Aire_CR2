...
def all_photo(cetesb_login, cetesb_password, start_date, end_date, station, csv_photo=False, path='./'):
    ...
    so2 =cetesb_data_download(cetesb_login, cetesb_password, start_date, end_date, 13, station) 
    ...
    if csv_photo:
        all_photo_df.to_csv(path+'all_photo_' + str(station) + '.csv',
                            index_label='date')
    else:
        all_photo_df.to_pickle(path+'all_photo_'+str(station)+ '.pkl')
...
def all_met(cetesb_login, cetesb_password,  start_date, end_date, station, rm_flag = True, csv_met=False, path='./'):
    ...
    # Export to csv
    if csv_met:
        all_met_df.to_csv(path+'all_met_' + str(station) + '.csv',
                            index_label='date')
    else:
        all_met_df.to_pickle(path+'all_met_'+ str(station) + '.pkl')
