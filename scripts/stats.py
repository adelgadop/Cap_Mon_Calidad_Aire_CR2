...
def pol_stats(data:pd.DataFrame, pol:str, suf:list[str]=['_obs','_mod']) -> dict:
    '''
    ...
    NMB  = {}  # Normalized Mean Bias in %
    NME  = {}  # Normalized Mean Error in %
    ...
    df  = data[[pol+suf[0],pol+suf[1]]].dropna()
    obs = df[pol+suf[0]]
    mod = df[pol+suf[1]]

    n    = obs.count()
    MB   = mod.mean()-obs.mean()
    MAGE = (mod-obs).abs().mean()
    RMSE = (((mod - obs)**2).mean())**0.5
    NMB  = (mod - obs).sum() / obs.sum() * 100
    NME  = (mod - obs).abs().sum() / obs.sum() * 100
    A = ((mod - obs)**2).sum()
    ...
    res = {'n':n,'MB':MB,'MAGE':MAGE,'RMSE':RMSE, 'NMB':NMB,'NME':NME,
           'IOA':IOA,'r':r,'Mm':Mm, 'Om':Om,'Msd':Msd,'Osd':Osd}
              
    return res

def aq_stats(data:pd.DataFrame, polls:list[str], suf:list[str]=['_obs','_mod'], 
             by_code: bool = False) -> pd.DataFrame:
    '''
    Statistics for model evaluation for air quality parameters, according to Emery et al. (2017). 
    Parameters
    ----------
    ...
    if by_code:
        ...
        for pol in polls:
            ...
            for code in codes:
                ...
                code_dic[code] = pol_stats(df, pol, suf)
            pol_dic[pol] = pd.DataFrame(code_dic)
        df = pd.concat(pol_dic, axis = 1).round(2).T
        df.index.names = ['Pollutant', 'code']
        # df.xs(value, level=0)  # level could be 0 or 1
            
    else:
        for pol in polls:
            pol_dic[pol] = pol_stats(data, pol, suf)
        df = pd.DataFrame(pol_dic).round(2)
...
def met_stats(data: pd.DataFrame, mets: list[str] = ['tc', 'rh', 'ws', 'wd'],
              suf: list[str] = ['_obs', '_mod']):
    '''
    ...
    for met in mets:
        df        = data.loc[:,[met+suf[0],met+suf[1]]].dropna()
        obs       = df[met+suf[0]]
        mod       = df[met+suf[1]]
