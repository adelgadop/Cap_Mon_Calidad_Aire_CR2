...
for c,n in zip(cetesb_stations.code,cetesb_stations.name):
    ...
    qr.all_photo(cetesb_login, cetesb_password, start_date, end_date,
                 c, csv_photo=False, path='./')
    print('Downloading meteo ' + n + ' Station')
    qr.all_met(cetesb_login, cetesb_password,  start_date, end_date,
               c, csv_met=False, path='./')
